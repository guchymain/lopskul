...
const links = [
  { to: '/instructor', label: 'Overview', icon: 'chart' },
  { to: '/instructor/courses', label: 'Courses', icon: 'layers' },
]
  ...
  return (
    <div className="min-h-screen bg-paper text-ink flex">
      <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-border bg-surface-1 p-4">
        ...
        <p className="px-2 mb-6 text-caption text-ink-soft uppercase tracking-wide">Instructor studio</p>
        <nav className="flex flex-col gap-1 flex-1" aria-label="Instructor navigation">
          {links.map((link) => (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden flex items-center justify-between border-b border-border bg-paper px-4 h-14">
          <Link to="/" className="font-display font-semibold text-heading-3">Ledger · Instructor</Link>
        </header>
